...
public class TipoServicioImp implements TipoServicio {
	...
	private final String URL ="http://localhost:9090/api/v1/tipos/";

	@Override
	public List<Tipo> obtenerTodosTipo(String token) {
//		Header
...
//		Entrada
		...
		try {
//			Respuesta
			ResponseEntity<Tipo[]> response = restTemp.exchange(
					URL,
					HttpMethod.GET,
					entity,
					Tipo[].class
	...
	@Override
	public Tipo insertarTipo(String token, Tipo tipo) {
		HttpHeaders headers = new HttpHeaders();
		headers.setBearerAuth(token);
		
		HttpEntity<Tipo> entity = new HttpEntity<>(tipo, headers);
	...
	@Override
	public Tipo obtenerTipoPorId(String token, Integer id) {
		HttpHeaders headers = new HttpHeaders();
		headers.setBearerAuth(token);
	...
	@Override
	public Tipo modificarTipo(String token, Tipo tipo) {
		HttpHeaders headers = new HttpHeaders();
		headers.setBearerAuth(token);
	...
	@Override
	public Boolean borrarTipo(String token, Integer id) {
		HttpHeaders headers = new HttpHeaders();
		headers.setBearerAuth(token);
